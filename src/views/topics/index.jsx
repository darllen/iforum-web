...
import {criarResposta, fetchRespostasPorPerguntaId} from "../../services/resposta";
import {criarComentario, fetchComentariosPorRespostaId} from "../../services/comentario";
import {getUser} from "../../helpers/authStore";
import {fetchUsuarioPorId} from "../../services/usuario";
import {toggleCurtida, verificarCurtidasUsuario} from "../../services/curtidas/curtidas";
...
export default function Topic() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const respostasCurtidas = await verificarCurtidasUsuario(user.id, id);
                // Atualiza o estado das respostas para incluir a informação de curtida
                const updatedRespostas = fetchedRespostas.map((resposta) => ({
                    ...resposta,
                    isLiked: respostasCurtidas.includes(resposta.id)  // Marca se o usuário curtiu a resposta
                }));
    ...
    const handleToggleCurtida = async (respostaId) => {
        try {
            // Chama o serviço para alternar curtidas
            const curtidasAtualizadas = await toggleCurtida(user.id, respostaId);

            if (curtidasAtualizadas !== null) {
                // Atualiza o número de curtidas na resposta no estado
                const updatedRespostas = respostas.map((resposta) =>
                    resposta.id === respostaId ? {...resposta, curtidas: curtidasAtualizadas} : resposta
                );
                setRespostas(updatedRespostas);
            ...
            <div style={{display: 'flex', justifyContent: 'center'}}>
                ...
                <main>
                    ...
                    <RespostaList
                        ...
                        usuarios={usuarios}
                        handleToggleCurtida={handleToggleCurtida}
